...
class CHUDDetectionInfo : public CHudElement, public vgui::Panel
{
	DECLARE_CLASS_SIMPLE(CHUDDetectionInfo, vgui::Panel );
public:
	CHUDDetectionInfo( const char *pElementName );
	void Init( void );
	void VidInit( void );
...
DECLARE_HUDELEMENT( CHUDDetectionInfo );

CHUDDetectionInfo::CHUDDetectionInfo( const char *pElementName ) :
	CHudElement( pElementName ), BaseClass( NULL, "HUDDetectionInfo" )
{
...
void CHUDDetectionInfo::ApplySchemeSettings( IScheme *scheme )
{
...
void CHUDDetectionInfo::Init( void )
{
	m_ammoFade		= 0.0f;
	m_healthFade	= 0.0f;
...
void CHUDDetectionInfo::VidInit( void )
{
	Init();
...
void CHUDDetectionInfo::DrawWarning( int x, int y, CHudTexture *icon, float &time )
{
...
//-----------------------------------------------------------------------------
bool CHUDDetectionInfo::ShouldDraw( void )
{
...
//-----------------------------------------------------------------------------
void CHUDDetectionInfo::OnThink()
{
	BaseClass::OnThink();
...
void CHUDDetectionInfo::Paint()
{
	...
	// Check our detection for a warning
	int	currentDetection = player->m_iDetection;
	if ( currentDetection != m_lastHealth )
	{
		UpdateEventTime();
		m_lastHealth = currentDetection;

		if ( currentDetection >= DETECTION_WARNING_THRESHOLD )
		{
	...
	// Check our detection for a warning
	if ( currentDetection != m_lastAmmo )
	{
		UpdateEventTime();
		m_lastAmmo	= currentDetection;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHUDDetectionInfo::UpdateEventTime( void )
{
...
//-----------------------------------------------------------------------------
bool CHUDDetectionInfo::EventTimeElapsed( void )
{
